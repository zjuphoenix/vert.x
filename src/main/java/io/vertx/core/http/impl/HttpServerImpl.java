...
public class HttpServerImpl implements HttpServer, Closeable, MetricsProvider {
    ...
    public synchronized HttpServer listen(int port, String host, Handler<AsyncResult<HttpServer>> listenHandler) {
        ...
        synchronized (vertx.sharedHttpServers()) {
            ...
            if (shared == null || port == 0) {
                    ...
                    protected void initChannel(Channel ch) throws Exception {
                        ...
                        if (sslHelper.isSSL()) {
                            ...
                            if (options.isUseAlpn()) {
                                ...
                                /**
                                 * 这里会初始化pipeline，把http编解码的handler和路由handler加进去
                                 */
                                configureHttp1(pipeline);
                            }
                        } else {
                ...
                /**
                 * 这里为reqHandlerManager添加handler，而且会为availableWorkers添加eventloop，添加的eventloop是从这里传入的listenContext.nettyEventLoop()方法得到的。
                 * 因此每个HttpServer的worker EventloopGroup只包含1个eventloop。
                 * 因此如果只部署了1个verticle，那么只用到了1个eventloop监听网络读写事件，这在standard模式这个eventloop还会执行部分业务逻辑，这回成为性能瓶颈，因此需要部署多个verticle把eventloop分担完或者多使用阻塞代码块执行。
                 */
                ...
                /**
                 * 当通过verticle创建HttpServer并且部署多个verticle时，每次创建HttpServer发现是共享的server，
                 * 再执行addHandlers(this, listenContext)时这个listenerContext又传入了新的eventloop，相当于增加了netty worker eventloop的数量用于处理网络读写。
                 */
    ...
    private void addHandlers(HttpServerImpl server, ContextImpl context) {
        if (requestStream.handler() != null) {
            /**
             * 对于部署多个相同verticle，每个verticle创建1个httpserver的情况，这里requestStream.handler()得到的都是同一个handler，connectionHandler也是，
             * 只是context会不同，每次部署1个verticle，会从eventloopGroup中按顺序选择1个eventloop创建context。
             */
